...
import { Input } from '@hotel/ui/ui/input';
import {
    Table,
    TableBody,
    TableCell,
...
export default function GostiContent({
  ...
  // Cast to any to access all required keys without TypeScript errors
  ...
  return (
    <>
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead className="text-center">{t('titula') || 'Titula'}</TableHead>
              <TableHead className="text-center">{t('ime') || 'Ime'}</TableHead>
              <TableHead className="text-center">{t('prezime') || 'Prezime'}</TableHead>
              <TableHead className="text-center">{t('drzava') || 'Država'}</TableHead>
              <TableHead className="text-center">{t('email') || 'Email'}</TableHead>
              <TableHead className="text-center">{t('telefon') || 'Telefon'}</TableHead>
              ...
              <TableRow key={gost.id}>
                ...
                <TableCell>
                  <div className="flex items-center justify-center gap-2">
                    <Button asChild variant="secondary" size="sm" aria-label={t_any.uredi_gosta} title={t_any.uredi_gosta}>
                      <a href={`/gosti/izmeni?id=${gost.id}`}>
                        <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <Button
                      ...
                      variant="destructive"
                      size="sm"
                      aria-label={t_any.obrisi_gosta}
                      title={t_any.obrisi_gosta}
                    >
                      <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div className="flex flex-col gap-4 sm:hidden">
        {gosti.map((gost: any) => (
          <div key={gost.id} className="rounded-lg border bg-card shadow-sm p-4 flex flex-col gap-2">
            ...
            <div className="flex justify-between">
              <span className="font-semibold">{t_any.titula}:</span>
              <span>{gost.titula}</span>
            </div>
            <div className="flex justify-between">
              <span className="font-semibold">{t_any.ime}:</span>
              <span>{gost.ime}</span>
            </div>
            <div className="flex justify-between">
              <span className="font-semibold">{t_any.prezime}:</span>
              <span>{gost.prezime}</span>
            </div>
            <div className="flex justify-between">
              <span className="font-semibold">{t_any.drzava}:</span>
              <span>{gost.drzava}</span>
            </div>
            <div className="flex justify-between">
              <span className="font-semibold">{t_any.email}:</span>
              <span>{gost.email}</span>
            </div>
            <div className="flex justify-between">
              <span className="font-semibold">{t_any.telefon}:</span>
      ...
      <ConfirmDialog
        ...
        message={t_any.confirmDeleteMessage || 'Da li ste sigurni da želite da obrišete ovog gosta? Ova akcija se ne može poništiti.'}
