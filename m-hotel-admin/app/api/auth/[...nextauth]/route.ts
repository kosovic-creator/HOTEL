...
export const authOptions: NextAuthOptions = {
  ...
  callbacks: {
    async jwt({ token, user }) {
      if (user) {
        const extendedToken = token as JWT & { id?: string; uloga?: string | null };
        const extendedUser = user as { id?: string; uloga?: string | null };
        extendedToken.id = extendedUser.id;
        extendedToken.uloga = extendedUser.uloga ?? null;
      }
      return token;
    },
    async session({ session, token }) {
      if (session.user) {
        const extendedToken = token as JWT & { id?: string; uloga?: string | null };
        const sessionUser = session.user as { id?: string; uloga?: string | null };
        sessionUser.id = extendedToken.id;
        sessionUser.uloga = extendedToken.uloga ?? null;
      }
      return session;
    },
  },
