...
import { Tabs, TabsContent, TabsList, TabsTrigger } from '@hotel/ui/ui/tabs';
...
export default function RezervacijeContent({
  ...
  const formatPrice = (value: number) =>
    new Intl.NumberFormat(lang === 'sr' ? 'me-ME' : 'en-US').format(value);
      ...
      <Tabs defaultValue="table" className="w-full">
        <TabsList className="grid w-full max-w-2xl grid-cols-3">
          ...
          <TabsTrigger value="calendar">
            ...
            {t('calendar_view') || 'Kalendar'}
          </TabsTrigger>
          <TabsTrigger value="free-rooms">
            <svg className="mr-2 h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 6h16M4 10h16M4 14h16M4 18h16" />
            </svg>
            {t('free_rooms_tab') || t('free_rooms') || 'Slobodne sobe'}
          </TabsTrigger>
        </TabsList>
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead className="text-center">{t('guest_name') || 'Ime Gosta'}</TableHead>
                  <TableHead className="text-center">{t('room') || 'Soba'}</TableHead>
                  <TableHead className="text-center">{t('checkin_date') || 'Datum Prijave'}</TableHead>
                  <TableHead className="text-center">{t('checkout_date') || 'Datum Odjave'}</TableHead>
                  <TableHead className="text-center">{t('number_of_guests_label') || 'Broj Gostiju'}</TableHead>
                  <TableHead className="text-center">{t('popust') || 'Popust (%)'}</TableHead>
                  <TableHead className="text-center">{t('ukupna_cena') || 'Ukupna Cena'}</TableHead>
                  ...
                  return (
                    <TableRow key={rezervacija.id}>
                      <TableCell className="text-center font-mono">
                        <Link
                          href={`/rezervacije/${rezervacija.id}`}
