  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      {onlyUrls ? (
        <div className="flex flex-col gap-2">
          <textarea
            className="min-h-24 rounded-md border border-gray-300 p-3 text-sm"
            placeholder="Unesite URL-ove slika, odvojene zarezom"
            value={urlInput}
            onChange={(e) => {
              const nextValue = e.target.value;
              setUrlInput(nextValue);
              setImages(nextValue.split(',').map((img) => img.trim()).filter(Boolean));
            }}
          />
        </div>
      ) : (
